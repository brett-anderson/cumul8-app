...
var Add = React.createClass({displayName: "Add",
  ...
  validateForm: function(e) {
    var that = this;
    e.preventDefault();
    console.log(!this.state.emailValid || !this.state.timeValid || this.state.type == false);
    if(!this.state.emailValid || !this.state.timeValid || this.state.type == false) {
      this.setState({loading: true});
      setTimeout(function(){
        that.transitionTo('valid', that.state);
      }, 1000);
    }
  },
      ...
      React.createElement("div", {className: "row"}, 
        React.createElement("form", {className: "col-md-4 col-centered box"}, 
          ...
          React.createElement("div", {className: "form-actions clearfix"}, 
            ...
            React.createElement("button", {className: "btn btn-primary pull-right", onClick: this.validateForm}, "Submit")
          )
        )
      )
    )
...
// disabled={!this.state.emailValid || !this.state.timeValid || this.state.type == false}
module.exports = Add;